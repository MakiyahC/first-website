...
_.each = function (collection, action) {
    // array check
    // if yes, use for loop
    // > call action with element, index, array
    //else, use for in loop
    // call action with value, key, object
...
_.reject = function (collection, func) {
  const filtered = [];
...
_.pluck = function(array, key) {
  return _.map(array, function(obj, pos, array) {
    return obj[key];
  });
};
